...
import copy, json, subprocess, re, shutil, sys, os
...
assert parse_value_line(after_line_pattern, "d// after:").error
assert parse_value_line(after_line_pattern, "// after: bla").get_values() == ["bla"]
assert parse_value_line(after_line_pattern, " // after: bla").get_values() == ["bla"]
assert parse_value_line(after_line_pattern, "//  after: bla").get_values() == ["bla"]
assert parse_value_line(after_line_pattern, "//  after: c++").get_values() == ["c++"]
assert parse_value_line(after_line_pattern, "//  after: c++17").get_values() == ["c++17"]
assert parse_value_line(after_line_pattern, "// after: bla foo").get_values() == ["bla", "foo"]
assert parse_value_line(after_line_pattern, "// after: bla  foo").get_values() == ["bla", "foo"]
assert parse_value_line(after_line_pattern, "// after: bla \t foo").get_values() == ["bla", "foo"]
...
class VirtualFileSystem:
  ...
  def mount_file(self, source_file, target_dir, file_name = 'module.modulemap'):
    ...
    self.append_file(source_file, cache_file, self.has_target_path(target_dir))
    if not self.has_target_path(target_dir):
      new_entry = {}
      new_entry["name"] = str(target_dir)
      new_entry["type"] = "directory"
      new_entry["contents"] = [{ 'name': file_name, 'type': 'file', 'external-contents': str(cache_file)}]
...
class ClangModules:
  ...
  def invoke_clang(self, suffix):
    ...
    if out_encoding is None:
      out_encoding = 'utf-8'
    try:
      output = subprocess.check_output("LANG=C " + self.clang_invok + " " + suffix, stderr=subprocess.STDOUT, shell=True)
    ...
    for modulemap_dir in self.modulemap_dirs:
      for filename in os.listdir(modulemap_dir):
        if not filename.endswith(".modulemap"):
          continue
        file_path = os.path.abspath(os.path.sep.join([modulemap_dir, filename]))
  ...
  def calculate_include_paths(self, make_abs = True):
    ...
    if output.exit_code != 0:
      raise NameError('Clang failed with non-zero exit code: ' + str(output.output))
...
for i in range(0, len(sys.argv)):
  ...
  if parsing_invocation:
    ...
    if arg == "--invocation":
      ...
      if output_dir:
        arg_parse_error("specified multiple output dirs with --output-dir")
      output_dir = next_arg
      parsed_arg = True
    else:
